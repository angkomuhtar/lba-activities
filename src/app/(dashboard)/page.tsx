...
export default async function DashboardPage({
  ...
  const count = (s: "hijau" | "kuning" | "merah" | null) =>
    data.filter(({ latest }) =>
      s === null ? latest === null : latest?.status === s,
    ).length;
  ...
  const pageNum =
    Number.isFinite(parsed) && parsed > 0
      ? Math.min(Math.floor(parsed), perShipTotalPages)
      : 1;
  const perShipRows = perShip.slice(
    (pageNum - 1) * PAGE_SIZE,
    pageNum * PAGE_SIZE,
  );
  ...
  const unpaidTotalPages = Math.max(
    1,
    Math.ceil(unpaid.length / UNPAID_PAGE_SIZE),
  );
  ...
  const unpaidPageNum =
    Number.isFinite(parsedUnpaid) && parsedUnpaid > 0
      ...
      : 1;
  const unpaidRows = unpaid.slice(
    (unpaidPageNum - 1) * UNPAID_PAGE_SIZE,
    unpaidPageNum * UNPAID_PAGE_SIZE,
  );
      ...
      <div className='grid gap-6 lg:grid-cols-2'>
        <Card className=''>
          ...
          <CardContent>
            {alerts.length === 0 ? (
              <p className='text-sm text-muted-foreground'>
                Tidak ada kapal dengan aktivitas merah (2 hari, khusus Waiting
                Dokumen 3 hari).
              </p>
            ) : (
                  ...
                  <Link
                    ...
                    <div className='min-w-0 flex-1'>
                      ...
                      <p className='text-xs text-muted-foreground'>
                        {formatDate(alert.from)} — {formatDate(alert.to)} ·{" "}
                        {alert.days} hari
                      </p>
                    </div>
          ...
          <Card>
            ...
            <CardContent>
              {expiringDocs.length === 0 ? (
                <p className='text-sm text-muted-foreground'>
                  Tidak ada dokumen yang akan kedaluwarsa dalam 30 hari ke
                  depan.
                </p>
              ) : (
          ...
          <CardContent>
            {perShip.length === 0 ? (
              ...
              <div className='overflow-x-auto'>
                ...
                {perShip.length > 0 && (
                  <Pagination page={pageNum} totalPages={perShipTotalPages} />
                )}
              </div>
            )}
          </CardContent>
